...
use palisade_config::{Config, PolicyConfig, RootTag};
...
proptest! {
    ...
    fn tag_derivation_is_deterministic(
        ...
        let tag1 = root.derive_artifact_tag_bytes(&hostname, &artifact_id);
        let tag2 = root.derive_artifact_tag_bytes(&hostname, &artifact_id);
    ...
    fn different_hostnames_different_tags(
        ...
        let tag1 = root.derive_artifact_tag_bytes(&hostname1, &artifact_id);
        let tag2 = root.derive_artifact_tag_bytes(&hostname2, &artifact_id);
    ...
    fn different_artifacts_different_tags(
        ...
        let tag1 = root.derive_artifact_tag_bytes(&hostname, &artifact1);
        let tag2 = root.derive_artifact_tag_bytes(&hostname, &artifact2);
    ...
    fn derived_tags_have_consistent_length(
        ...
        let root = RootTag::generate().unwrap();
        let tag = root.derive_artifact_tag_bytes(&hostname, &artifact_id);

        // SHA3-512 produces 64 bytes in the fixed-size digest form.
...
proptest! {
    ...
    fn diff_symmetric_for_no_changes(_seed in any::<u64>()) {
        ...
        let changes1 = config1.diff(&config2).expect("config diff");
        let changes2 = config2.diff(&config1).expect("config diff");
