...
use palisade_config::{Config, ConfigChange, PolicyChange, PolicyConfig, RootTag, ValidationMode};
...
async fn test_config_roundtrip_serialization() {
    ...
    assert_eq!(
        loaded.deception.honeytoken_count,
        original.deception.honeytoken_count
    );
}
...
async fn test_policy_roundtrip_serialization() {
    ...
    assert_eq!(loaded.version, original.version);
    assert_eq!(
        loaded.scoring.alert_threshold,
        original.scoring.alert_threshold
    );
...
#[cfg(unix)]
#[tokio::test]
async fn test_policy_rejects_insecure_permissions() {
    use std::os::unix::fs::PermissionsExt;

    let temp_dir = TempDir::new().unwrap();
    let policy_path = temp_dir.path().join("world_readable_policy.toml");

    let policy = PolicyConfig::default();
    let toml_str = toml::to_string(&policy).unwrap();
    fs::write(&policy_path, &toml_str).await.unwrap();

    let mut perms = std::fs::metadata(&policy_path).unwrap().permissions();
    perms.set_mode(0o644);
    std::fs::set_permissions(&policy_path, perms).unwrap();
...
fn test_config_diff_detects_multiple_changes() {
    ...
    let changes = config1.diff(&config2).expect("config diff");

    // Should detect root tag change, path change, and capability change
    assert!(changes.len() >= 2); // At least root tag and one other

    let has_root_tag_change = changes
        .iter()
        .any(|c| matches!(c, ConfigChange::RootTagChanged { .. }));
    let has_path_change = changes
        .iter()
        .any(|c| matches!(c, ConfigChange::PathAdded { .. } | ConfigChange::PathRemoved { .. }));
...
fn test_policy_diff_detects_threshold_changes() {
    ...
    let changes = policy1.diff(&policy2).expect("policy diff");
...
async fn test_concurrent_config_loading() {
    ...
    for _ in 0..10 {
        ...
        let handle = tokio::spawn(async move { Config::from_file(&path).await });
        handles.push(handle);
    }
...
fn test_tag_derivation_consistency() {
    ...
    let tag3 = root.derive_artifact_tag_bytes("host2", "artifact1");
    assert_ne!(tag1, tag3);

    // Different artifact should produce different tag
    let tag4 = root.derive_artifact_tag_bytes("host1", "artifact2");
    assert_ne!(tag1, tag4);
}
...
fn test_tag_derivation_hierarchy() {
    ...
    let host_tag1 = root.derive_host_tag_bytes("production-server");
    let host_tag2 = root.derive_host_tag_bytes("production-server");
    assert_eq!(host_tag1, host_tag2);

    // Different roots should produce different host tags
    let root2 = RootTag::generate().unwrap();
    let host_tag3 = root2.derive_host_tag_bytes("production-server");
