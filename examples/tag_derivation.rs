...
fn main() {
    ...
    let root = RootTag::generate()
        .expect("OS RNG produced invalid entropy — catastrophic system failure");
    ...
    let hex_str = serde_json::to_string(&root_for_hex_demo)
        .expect("serialise")
        .trim_matches('"')
        .to_string();
    ...
    assert!(root_for_hex_demo.hash_eq_ct(&parsed),
    ...
    // Level 1: host tag
    let host_a = root.derive_host_tag("honeypot-alpha.internal");
    let host_b = root.derive_host_tag("honeypot-beta.internal");
    ...
    // Level 2: artifact tag (full allocation-based API)
    let art_a1 = root.derive_artifact_tag("honeypot-alpha.internal", "aws-credentials-001");
    let art_a2 = root.derive_artifact_tag("honeypot-alpha.internal", "aws-credentials-002");
    let art_b1 = root.derive_artifact_tag("honeypot-beta.internal",  "aws-credentials-001");

    println!("\n  artifact alpha/art-001 [:8]: {}...", &art_a1[..16]);
    println!("  artifact alpha/art-002 [:8]: {}...", &art_a2[..16]);
    println!("  artifact beta /art-001 [:8]: {}...", &art_b1[..16]);

    assert_ne!(art_a1, art_a2, "Same host, different artifact → different tag");
    assert_ne!(art_a1, art_b1, "Different host, same artifact → different tag");
    println!("  Isolation: all artifact tag collisions: NONE ✓");

    // Determinism check
    let art_a1_again = root.derive_artifact_tag("honeypot-alpha.internal", "aws-credentials-001");
    ...
    println!("  tag (128 hex chars): {}...{}", &as_str[..16], &as_str[112..]);
    ...
    let allocating = root.derive_artifact_tag("honeypot-alpha.internal", "ssh-key-honeypot");
    assert_eq!(allocating, as_str, "No-alloc and allocating paths must agree");
    println!("  Consistency (no-alloc == alloc): PASSED ✓");
    ...
    assert!(original.hash_eq_ct(&deserialised), "Round-trip must preserve tag identity");
    println!("  round-trip identity: PASSED ✓");

    // Deserialising "***REDACTED***" must fail — prevents log scraping attacks
    let redacted = r#""***REDACTED***""#;
    assert!(serde_json::from_str::<RootTag>(redacted).is_err(),
